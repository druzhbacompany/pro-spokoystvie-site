...
const NAV_V2 = ["Услуги", "Врачи", "Цены", "Контакты"];

/** V2 TopBar — donor brand (blue/beige, Manrope, double-ring logo). Sticky, condenses on scroll. */
export function TopBarV2() {
  ...
  return (
    ...
    >
      ...
      {open ? (
        <div id="v2-mobile-menu" className="border-t px-4 pb-6 pt-4 lg:hidden" style={{ borderColor: "var(--v2-border)" }}>
          <a href="#zayavka" onClick={() => setOpen(false)} className="v2-btn v2-btn-primary mb-4 w-full">
            Записаться
          </a>
          <nav aria-label="Мобильная навигация" className="flex flex-col">
            {NAV_V2.map((item) => (
              <a key={item} href="#" onClick={() => setOpen(false)} className="border-b py-3 text-lg" style={{ borderColor: "var(--v2-border)", color: "var(--v2-blue-deep)" }}>
                {item}
              </a>
            ))}
          </nav>
